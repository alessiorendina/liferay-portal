...
<%
CommerceCartMiniDisplayContext commerceCartMiniDisplayContext = (CommerceCartMiniDisplayContext)request.getAttribute(WebKeys.PORTLET_DISPLAY_CONTEXT);

Map<String, Object> contextObjects = HashMapBuilder.<String, Object>put(
	"commerceCartMiniDisplayContext", commerceCartMiniDisplayContext
).build();
...
String priceDisplayType = commerceCartMiniDisplayContext.getCommercePriceDisplayType();

CommerceOrderPrice commerceOrderPrice = commerceCartMiniDisplayContext.getCommerceOrderPrice();
...
SearchContainer<CommerceOrderItem> commerceOrderItemSearchContainer = commerceCartMiniDisplayContext.getSearchContainer();

PortletURL portletURL = PortletURLBuilder.create(
	commerceCartMiniDisplayContext.getPortletURL()
).setParameter(
...
<liferay-ddm:template-renderer
	className="<%= CommerceCartMiniPortlet.class.getName() %>"
	contextObjects="<%= contextObjects %>"
	displayStyle="<%= commerceCartMiniDisplayContext.getDisplayStyle() %>"
	displayStyleGroupId="<%= commerceCartMiniDisplayContext.getDisplayStyleGroupId() %>"
		...
		<li class="autofit-row">
			<div class="autofit-col autofit-col-expand">
				<div class="commerce-title h4">
					<liferay-ui:message arguments="<%= commerceCartMiniDisplayContext.getCommerceOrderItemsQuantity() %>" key="items-x" translateArguments="<%= false %>" />
				</div>
			</div>

			<div class="autofit-col">
				<liferay-commerce:order-transitions
					commerceOrderId="<%= commerceCartMiniDisplayContext.getCommerceOrderId() %>"
					cssClass="btn commerce-btn"
				/>
			</div>

			<c:if test="<%= commerceCartMiniDisplayContext.hasPermission(ActionKeys.VIEW) %>">
				<div class="autofit-col">
					<div><a class="btn commerce-btn" href="<%= commerceCartMiniDisplayContext.getCommerceCartPortletURL() %>"><liferay-ui:message key="edit-cart" /></a></div>
				</div>
			</c:if>
		</li>
	</ul>
	...
	<div class="commerce-order-items-body" id="<portlet:namespace />entriesContainer">
		...
		>
			...
			>
				...
				String cpInstanceCDNURL = commerceCartMiniDisplayContext.getCPInstanceCDNURL(commerceOrderItem);
				%>
							...
							<c:choose>
								...
								<c:otherwise>
									<liferay-adaptive-media:img
										alt="thumbnail"
										class="sticker-img"
										fileVersion="<%= commerceCartMiniDisplayContext.getCPInstanceImageFileVersion(commerceOrderItem) %>"
									/>
								</c:otherwise>
							</c:choose>
						</span>
				...
				>
					<div class="description-section">
						<div class="list-group-title">
							<a href="<%= commerceCartMiniDisplayContext.getCPDefinitionURL(cpDefinition.getCPDefinitionId(), themeDisplay) %>">
						...
						for (KeyValuePair keyValuePair : commerceCartMiniDisplayContext.getKeyValuePairs(commerceOrderItem.getCPDefinitionId(), commerceOrderItem.getJson(), locale)) {
				...
				<liferay-ui:search-container-column-text
					name="price"
				>
					<c:if test="<%= commerceCartMiniDisplayContext.hasViewPricePermission() %>">

						<%
						CommerceMoney unitPriceCommerceMoney = commerceCartMiniDisplayContext.getUnitPriceCommerceMoney(commerceOrderItem);
						CommerceMoney unitPromoPriceCommerceMoney = commerceCartMiniDisplayContext.getUnitPromoPriceCommerceMoney(commerceOrderItem);
						%>

						<c:choose>
							<c:when test="<%= commerceCartMiniDisplayContext.isUnitPromoPriceActive(commerceOrderItem) %>">
								<c:choose>
		...
		<c:if test="<%= commerceCartMiniDisplayContext.getCommerceOrderItemsQuantity() > commerceOrderItemSearchContainer.getDelta() %>">
			<ul class="commerce-order-items-header">
				<li class="autofit-row">
					<c:if test="<%= commerceCartMiniDisplayContext.hasPermission(ActionKeys.VIEW) %>">
						<a class="btn btn-link commerce-link" href="<%= commerceCartMiniDisplayContext.getCommerceCartPortletURL() %>"><liferay-ui:message key="view-more" /></a>
					</c:if>
				</li>
			</ul>
		</c:if>
	...
	<ul class="commerce-order-items-footer">
		<li class="autofit-row commerce-tax">
			...
			<c:if test="<%= subtotalCommerceDiscountValue != null %>">
				...
				<div class="commerce-value pl-1">
					(<%= HtmlUtil.escape(commerceCartMiniDisplayContext.getLocalizedPercentage(subtotalCommerceDiscountValue.getDiscountPercentage(), locale)) %>)
				</div>
			</c:if>
		</li>
		<li class="autofit-row commerce-tax">
		...
		<li class="autofit-row commerce-total">
			<c:if test="<%= totalCommerceDiscountValue != null %>">
				...
				<div class="commerce-value pl-1">
					(<%= HtmlUtil.escape(commerceCartMiniDisplayContext.getLocalizedPercentage(totalCommerceDiscountValue.getDiscountPercentage(), locale)) %>)
				</div>
			</c:if>
...
<liferay-frontend:component
	module="{cartMiniView} from commerce-cart-web"
/>