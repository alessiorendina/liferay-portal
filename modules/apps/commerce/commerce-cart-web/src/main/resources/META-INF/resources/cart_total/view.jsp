...
<%
CommerceCartTotalDisplayContext commerceCartTotalDisplayContext = (CommerceCartTotalDisplayContext)request.getAttribute(WebKeys.PORTLET_DISPLAY_CONTEXT);
...
String priceDisplayType = commerceCartTotalDisplayContext.getCommercePriceDisplayType();

CommerceOrderPrice commerceOrderPrice = commerceCartTotalDisplayContext.getCommerceOrderPrice();
...
Map<String, Object> contextObjects = HashMapBuilder.<String, Object>put(
	"commerceCartTotalDisplayContext", commerceCartTotalDisplayContext
).build();

SearchContainer<CommerceOrderItem> commerceOrderItemSearchContainer = commerceCartTotalDisplayContext.getSearchContainer();
%>

<liferay-ddm:template-renderer
	className="<%= CommerceCartTotalPortlet.class.getName() %>"
	contextObjects="<%= contextObjects %>"
	displayStyle="<%= commerceCartTotalDisplayContext.getDisplayStyle() %>"
	displayStyleGroupId="<%= commerceCartTotalDisplayContext.getDisplayStyleGroupId() %>"
...
>
	...
	<aui:button-row>
		...
		<%
		PortletURL checkoutPortletURL = commerceCartTotalDisplayContext.getCheckoutPortletURL();
		CommerceOrder commerceOrder = commerceCartTotalDisplayContext.getCommerceOrder();
		%>

		<c:choose>
			<c:when test="<%= (commerceOrder != null) && (commerceOrder.getStatus() != 0) && commerceCartTotalDisplayContext.hasPermission(ActionKeys.UPDATE) && commerceCartTotalDisplayContext.isValidCommerceOrder() %>">
				<liferay-commerce:order-transitions
					commerceOrderId="<%= commerceCartTotalDisplayContext.getCommerceOrderId() %>"
					cssClass="btn btn-fixed btn-primary"
				/>
			</c:when>
			<c:otherwise>
				<aui:button cssClass="btn-fixed" disabled="<%= !commerceCartTotalDisplayContext.isValidCommerceOrder() %>" href="<%= checkoutPortletURL.toString() %>" type="submit" value="checkout" />
			</c:otherwise>
		</c:choose>

		<c:if test="<%= commerceCartTotalDisplayContext.isRequestQuoteEnabled() && commerceCartTotalDisplayContext.isValidCommerceOrder() %>">
	...
	<liferay-frontend:component
		module="{cartTotalView} from commerce-cart-web"
	/>
</liferay-ddm:template-renderer>