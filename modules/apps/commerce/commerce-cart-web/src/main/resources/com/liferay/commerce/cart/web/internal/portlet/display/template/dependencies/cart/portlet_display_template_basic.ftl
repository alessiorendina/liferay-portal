		...
		<#if entries?has_content>
			...
			<#list entries as curCommerceOrderItem>
				<#assign
					...
					productURL = commerceCartDisplayContext.getCPDefinitionURL(cpDefinition.getCPDefinitionId(), themeDisplay)

					name = cpDefinition.getName(themeDisplay.getLanguageId())

					deleteURL = commerceCartDisplayContext.getDeleteURL(curCommerceOrderItem)
				/>

				<#if commerceCartDisplayContext.getCommerceOrderItemThumbnailSrc(curCommerceOrderItem)??>
					<#assign
						image = commerceCartDisplayContext.getCommerceOrderItemThumbnailSrc(curCommerceOrderItem)
					/>
				</#if>
