...
<#if entries?has_content>
	<div class="row">
		<#assign
			orderPrice = commerceCartMiniDisplayContext.getCommerceOrderPrice()
		...
		<#list entries as curCommerceOrderItem>
			<#assign
				...
				productURL = commerceCartMiniDisplayContext.getCPDefinitionURL(cpDefinition.getCPDefinitionId(), themeDisplay)

				name = cpDefinition.getName(themeDisplay.getLanguageId())
			/>

			<#if commerceCartMiniDisplayContext.getCommerceOrderItemThumbnailSrc(curCommerceOrderItem)??>
				<#assign
					image = commerceCartMiniDisplayContext.getCommerceOrderItemThumbnailSrc(curCommerceOrderItem)
				/>
			</#if>
