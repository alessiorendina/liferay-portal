/**
 * SPDX-FileCopyrightText: (c) 2000 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

package com.liferay.commerce.order.web.internal.constants;

import com.liferay.commerce.constants.CommercePortletKeys;

/**
 * @author Marco Leo
 * @author Alessio Antonio Rendina
 */
public class CommerceOrderFDSNames {

	public static final String BILLING_ADDRESSES =
		CommercePortletKeys.COMMERCE_ORDER + "-billingAddress";

	public static final String DRAFT_RETURN_ITEMS =
		CommercePortletKeys.COMMERCE_RETURN + "-draftReturnItems";

	public static final String IMPORT_ORDERS =
		CommercePortletKeys.COMMERCE_OPEN_ORDER + "-importOrders";

	public static final String PENDING_ORDER_ITEMS =
		CommercePortletKeys.COMMERCE_OPEN_ORDER + "-pendingOrderItems";

	public static final String PENDING_ORDERS =
		CommercePortletKeys.COMMERCE_OPEN_ORDER + "-pendingOrders";

	public static final String PLACED_ORDER_ITEMS =
		CommercePortletKeys.COMMERCE_ORDER + "-placedOrderItems";

	public static final String PLACED_ORDERS =
		CommercePortletKeys.COMMERCE_ORDER + "-placedOrders";

	public static final String PREVIEW_ORDER_ITEMS =
		CommercePortletKeys.COMMERCE_OPEN_ORDER + "-previewOrderItems";

	public static final String REFUND_ITEMS =
		CommercePortletKeys.COMMERCE_RETURN + "-refundItems";

	public static final String RETURN_ITEMS =
		CommercePortletKeys.COMMERCE_RETURN + "-returnItems";

	public static final String RETURNABLE_ORDER_ITEMS =
		CommercePortletKeys.COMMERCE_ORDER + "-returnableOrderItems";

	public static final String RETURNS =
		CommercePortletKeys.COMMERCE_RETURN + "-returns";

	public static final String SHIPMENTS =
		CommercePortletKeys.COMMERCE_ORDER + "-shipments";

	public static final String SHIPPING_ADDRESSES =
		CommercePortletKeys.COMMERCE_ORDER + "-shippingAddress";

	public static final String WISH_LISTS =
		CommercePortletKeys.COMMERCE_OPEN_ORDER + "-wishLists";

}