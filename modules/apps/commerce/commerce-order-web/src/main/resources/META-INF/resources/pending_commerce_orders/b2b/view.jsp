...
<liferay-ddm:template-renderer
	className="<%= CommerceOpenOrderPortlet.class.getName() %>"
	contextObjects='<%=
		HashMapBuilder.<String, Object>put(
			"commerceOrderDisplayContext", commerceOrderDisplayContext
		).build()
	%>'
	displayStyle="<%= commerceOrderDisplayContext.getDisplayStyle(CommercePortletKeys.COMMERCE_OPEN_ORDER_CONTENT) %>"
	displayStyleGroupId="<%= commerceOrderDisplayContext.getDisplayStyleGroupId(CommercePortletKeys.COMMERCE_OPEN_ORDER_CONTENT) %>"
...
>
	...
	<div class="commerce-cta is-visible">
		<c:if test="<%= commerceOrderDisplayContext.hasPermission(accountEntry, CommerceOrderActionKeys.ADD_COMMERCE_ORDER) %>">
			...
			<c:if test="<%= commerceOrderDisplayContext.getCommerceOrderTypesCount() > 1 %>">
				<portlet:renderURL var="viewCommerceOrderOrderTypeURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
					<portlet:param name="mvcRenderCommandName" value="/commerce_order/view_commerce_order_order_type_modal" />
				</portlet:renderURL>
