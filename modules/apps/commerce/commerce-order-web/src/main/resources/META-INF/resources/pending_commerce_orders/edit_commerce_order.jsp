...
CommerceOrderPrice commerceOrderPrice = commerceOrderDisplayContext.getCommerceOrderPrice();
...
String priceDisplayType = commerceOrderDisplayContext.getCommercePriceDisplayType();
...
AccountEntry accountEntry = commerceOrderDisplayContext.getAccountEntry();

if (commerceOrder != null) {
	accountEntry = commerceOrder.getAccountEntry();
}

List<CommerceAddress> shippingAddresses = commerceOrderDisplayContext.getShippingCommerceAddresses(accountEntry.getAccountEntryId());
List<CommerceAddress> billingAddresses = commerceOrderDisplayContext.getBillingCommerceAddresses(accountEntry.getAccountEntryId());
...
<portlet:actionURL name="/commerce_open_order/edit_commerce_order" var="editCommerceOrderActionURL">
	<portlet:param name="mvcRenderCommandName" value="/commerce_open_order/edit_commerce_order" />
</portlet:actionURL>
...
<aui:form action="<%= editCommerceOrderActionURL %>" cssClass="order-details-container" method="post" name="fm">
			...
			<div class="align-items-center row">
				...
				<div class="col-md-3">
					<dl class="commerce-list">
						...
						<dd><%= commerceOrderDisplayContext.getCommerceOrderTypeName(languageId) %></dd>
					</dl>
				</div>
				...
				<div class="col-md-3">
					<dl class="commerce-list">
						...
						<dd>
							<%= commerceOrderDisplayContext.getCommerceOrderDate(commerceOrder) %>
							<%= commerceOrderDisplayContext.getCommerceOrderTime(commerceOrder) %>
						</dd>
					</dl>
				</div>
			</div>
	...
	<c:choose>
		<c:when test="<%= commerceOrderDisplayContext.isShowPurchaseOrderNumber() %>">
			<div class="row">
				<div class="col-md-6">
					<div class="commerce-panel">
						...
						<div class="commerce-panel__content">
							<dl class="commerce-list">
								<c:choose>
									<c:when test="<%= commerceOrderDisplayContext.hasModelPermission(commerceOrder, ActionKeys.UPDATE) %>">
	...
	>
		...
		PortletURL viewCommerceOrderImporterTypeURL = PortletURLBuilder.createRenderURL(
			...
			"/commerce_open_order/view_commerce_order_importer_type"
		).setParameter(
		...
		for (CommerceOrderImporterType commerceOrderImporterType : commerceOrderDisplayContext.getCommerceImporterTypes(commerceOrder)) {
		...
		<c:if test="<%= commerceOrderDisplayContext.hasModelPermission(commerceOrder, ActionKeys.DELETE) %>">
			<portlet:actionURL name="/commerce_open_order/edit_commerce_order" var="deleteURL">
			...
			<portlet:actionURL name="/commerce_open_order/edit_commerce_order_item" var="deleteOrderURL">
			...
			<liferay-ui:icon
				...
				message="print"
				target="_blank"
				url="<%= commerceOrderDisplayContext.getExportCommerceOrderReportURL() %>"
			/>
		</c:if>
	</liferay-ui:icon-menu>

	<div class="commerce-cta is-visible">
		<c:if test="<%= commerceOrderDisplayContext.hasModelPermission(commerceOrder, ActionKeys.UPDATE) %>">
			<clay:button
		...
		<c:if test="<%= commerceOrderDisplayContext.isRequestQuoteButtonEnabled() %>">
			...
			<div class="commerce-panel__content">
				<dl class="commerce-list">
					...
					<c:if test="<%= (subtotalCommerceDiscountValue != null) && (BigDecimal.ZERO.compareTo(subtotalCommerceDiscountValue.getDiscountPercentage()) < 0) %>">
						...
						<dt></dt>
						<dd class="text-right"><%= HtmlUtil.escape(commerceOrderDisplayContext.getLocalizedPercentage(subtotalCommerceDiscountValue.getDiscountPercentage(), locale)) %></dd>
					</c:if>
					...
					<c:if test="<%= (shippingCommerceDiscountValue != null) && (BigDecimal.ZERO.compareTo(shippingCommerceDiscountValue.getDiscountPercentage()) < 0) %>">
						...
						<dt></dt>
						<dd class="text-right"><%= HtmlUtil.escape(commerceOrderDisplayContext.getLocalizedPercentage(shippingCommerceDiscountValue.getDiscountPercentage(), locale)) %></dd>
					</c:if>
					...
					<c:if test="<%= (totalCommerceDiscountValue != null) && (BigDecimal.ZERO.compareTo(totalCommerceDiscountValue.getDiscountPercentage()) < 0) %>">
						...
						<dt></dt>
						<dd class="text-right"><%= HtmlUtil.escape(commerceOrderDisplayContext.getLocalizedPercentage(totalCommerceDiscountValue.getDiscountPercentage(), locale)) %></dd>
					</c:if>
				</dl>
			</div>
...
<portlet:actionURL name="/commerce_open_order/edit_commerce_order" var="editCommerceOrderURL" />
