...
<div class="container-fluid container-xl mt-3">
	<div class="alert alert-info center container-fluid">
		...
		<div class="row-fluid">
			<aui:button href="<%= commerceOrderDisplayContext.getCSVTemplateDownloadURL() %>" name="downloadCSVTemplateButton" primary="<%= true %>" value="download-template" />
		</div>
	</div>

	<portlet:actionURL name="/commerce_open_order/import_csv" var="importCSVActionURL" />
	...
	<aui:form action="<%= importCSVActionURL %>" enctype="multipart/form-data" method="post" name="fm">
		...
		<aui:input name="commerceOrderId" type="hidden" value="<%= String.valueOf(commerceOrderDisplayContext.getCommerceOrderId()) %>" />
