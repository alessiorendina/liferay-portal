...
<liferay-portlet:renderURL var="editBillingAddressURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/commerce_open_order/edit_commerce_order_billing_address" />
	<portlet:param name="commerceOrderId" value="<%= String.valueOf(commerceOrderDisplayContext.getCommerceOrderId()) %>" />
</liferay-portlet:renderURL>

<liferay-portlet:renderURL var="selectBillingAddressURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/commerce_open_order/select_commerce_order_billing_address" />
	<portlet:param name="commerceOrderId" value="<%= String.valueOf(commerceOrderDisplayContext.getCommerceOrderId()) %>" />
</liferay-portlet:renderURL>
...
<liferay-portlet:renderURL var="viewDeliveryTermsURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/commerce_order/view_commerce_order_delivery_terms" />
	<portlet:param name="commerceOrderId" value="<%= String.valueOf(commerceOrderDisplayContext.getCommerceOrderId()) %>" />
</liferay-portlet:renderURL>
...
<liferay-portlet:renderURL var="editNameURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/commerce_open_order/edit_commerce_order_name" />
	<portlet:param name="commerceOrderId" value="<%= String.valueOf(commerceOrderDisplayContext.getCommerceOrderId()) %>" />
</liferay-portlet:renderURL>
...
<liferay-portlet:renderURL var="viewPaymentTermsURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/commerce_order/view_commerce_order_payment_terms" />
	<portlet:param name="commerceOrderId" value="<%= String.valueOf(commerceOrderDisplayContext.getCommerceOrderId()) %>" />
</liferay-portlet:renderURL>
...
<liferay-portlet:renderURL var="editPurchaseOrderNumberURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/commerce_open_order/edit_commerce_order_purchase_order_number" />
	<portlet:param name="commerceOrderId" value="<%= String.valueOf(commerceOrderDisplayContext.getCommerceOrderId()) %>" />
</liferay-portlet:renderURL>
...
<liferay-portlet:renderURL var="editRequestedDeliveryDateURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/commerce_open_order/edit_commerce_order_requested_delivery_date" />
	<portlet:param name="commerceOrderId" value="<%= String.valueOf(commerceOrderDisplayContext.getCommerceOrderId()) %>" />
</liferay-portlet:renderURL>
...
<liferay-portlet:renderURL var="editShippingAddressURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/commerce_open_order/edit_commerce_order_shipping_address" />
	<portlet:param name="commerceOrderId" value="<%= String.valueOf(commerceOrderDisplayContext.getCommerceOrderId()) %>" />
</liferay-portlet:renderURL>

<liferay-portlet:renderURL var="selectShippingAddressURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/commerce_open_order/select_commerce_order_shipping_address" />
	<portlet:param name="commerceOrderId" value="<%= String.valueOf(commerceOrderDisplayContext.getCommerceOrderId()) %>" />
</liferay-portlet:renderURL>
...
<div class="row">
	<div class="col-12 mb-4">
		<commerce-ui:header
			actions="<%= commerceOrderDisplayContext.getHeaderActionModels() %>"
			bean="<%= commerceOrder %>"
			beanIdLabel="id"
			dropdownItems="<%= commerceOrderDisplayContext.getDropdownItems() %>"
			externalReferenceCode="<%= commerceOrder.getExternalReferenceCode() %>"
			model="<%= CommerceOrder.class %>"
			thumbnailUrl="<%= commerceOrderDisplayContext.getCommerceAccountThumbnailURL() %>"
			title="<%= String.valueOf(commerceOrder.getCommerceOrderId()) %>"
			transitionPortletURL="<%= commerceOrderDisplayContext.getTransitionOrderPortletURL(commerceOrder) %>"
		/>
	</div>
	...
	<c:if test="<%= !commerceOrder.isOpen() %>">
		<div class="col-12 mb-4">
			<commerce-ui:step-tracker
				...
				steps="<%= commerceOrderDisplayContext.getOrderSteps() %>"
			/>
		</div>
	</c:if>
		...
		>
			<div class="row vertically-divided">
				<div class="col-xl-4">
					...
					>
						<%= HtmlUtil.escape(commerceOrderDisplayContext.fetchCommerceChannel().getName()) %>
					</commerce-ui:info-box>
				</div>
				...
				<div class="col-xl-4">
					...
					<c:if test="<%= commerceOrderDisplayContext.hasViewBillingAddressPermission(permissionChecker, accountEntry) %>">
						<commerce-ui:info-box
							actionLabel='<%= commerceOrderDisplayContext.hasModelPermission(commerceOrder, ActionKeys.UPDATE) ? LanguageUtil.get(request, (billingCommerceAddress == null) ? "add" : "edit") : "" %>'
							actionTargetId='<%= commerceOrderDisplayContext.hasModelPermission(commerceOrder, ActionKeys.UPDATE) ? "billing-address-modal" : "" %>'
							elementClasses="py-3"
						...
						>
							<c:choose>
								...
								<c:otherwise>
									...
									<p class="mb-0">
										<%= commerceOrderDisplayContext.getDescriptiveAddress(billingCommerceAddress) %>
									</p>
								</c:otherwise>
							</c:choose>
						</commerce-ui:info-box>
					...
					<commerce-ui:info-box
						actionLabel='<%= commerceOrderDisplayContext.hasModelPermission(commerceOrder, ActionKeys.UPDATE) ? LanguageUtil.get(request, (shippingCommerceAddress == null) ? "add" : "edit") : "" %>'
						actionTargetId='<%= commerceOrderDisplayContext.hasModelPermission(commerceOrder, ActionKeys.UPDATE) ? "shipping-address-modal" : "" %>'
						elementClasses="py-3"
					...
					>
						<c:choose>
							...
							<c:otherwise>
								...
								<p class="mb-0">
									<%= commerceOrderDisplayContext.getDescriptiveAddress(shippingCommerceAddress) %>
								</p>
							</c:otherwise>
						</c:choose>
					</commerce-ui:info-box>
					...
					<commerce-ui:info-box
						actionLabel='<%= (commerceOrderDisplayContext.hasManageCommerceOrderPaymentTermsPermission() && (commerceOrder.getPaymentCommerceTermEntryId() > 0)) ? LanguageUtil.get(request, "view") : null %>'
						actionTargetId="payment-terms-modal"
						actionUrl="<%= (commerceOrderDisplayContext.hasManageCommerceOrderPaymentTermsPermission() && (commerceOrder.getPaymentCommerceTermEntryId() > 0)) ? viewPaymentTermsURL : null %>"
						elementClasses="py-3"
					...
					<commerce-ui:info-box
						actionLabel='<%= (commerceOrderDisplayContext.hasManageCommerceOrderDeliveryTermsPermission() && (commerceOrder.getDeliveryCommerceTermEntryId() > 0)) ? LanguageUtil.get(request, "view") : null %>'
						actionTargetId="delivery-terms-modal"
						actionUrl="<%= (commerceOrderDisplayContext.hasManageCommerceOrderDeliveryTermsPermission() && (commerceOrder.getDeliveryCommerceTermEntryId() > 0)) ? viewDeliveryTermsURL : null %>"
						elementClasses="py-3"
				...
				<div class="col-xl-4">
					<c:if test="<%= commerceOrder.getOrderDate() != null %>">
						...
						>
							<%= commerceOrderDisplayContext.formatCommerceOrderDate(commerceOrder.getOrderDate()) %>
						</commerce-ui:info-box>
					</c:if>
					...
					>
						<c:choose>
							...
							<c:otherwise>
								<%= commerceOrderDisplayContext.formatCommerceOrderDate(requestedDeliveryDate) %>
							</c:otherwise>
						</c:choose>
					</commerce-ui:info-box>
					...
					>
						<%= HtmlUtil.escape(commerceOrderDisplayContext.getCommerceOrderTypeName(LanguageUtil.getLanguageId(locale))) %>
					</commerce-ui:info-box>

					<portlet:renderURL var="editCommerceOrderNotesURL">
						<portlet:param name="mvcRenderCommandName" value="/commerce_open_order/edit_commerce_order_notes" />
			...
			<liferay-frontend:component
				context='<%=
					HashMapBuilder.<String, Object>put(
						"commerceOrderId", commerceOrderDisplayContext.getCommerceOrderId()
					).put(
