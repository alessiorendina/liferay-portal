			...
			<div class="col-md-3">
				<dl class="commerce-list">
					...
					<dd>
						...
						<c:if test="<%= commerceOrderDisplayContext.isShowCommerceOrderCreateTime() %>">
		...
		<div class="align-items-center row">
			...
			<div class="col-md-3">
				<dl class="commerce-list">
					...
					<dd><%= commerceOrderDisplayContext.getCommerceOrderStatusLabel(commerceOrder) %></dd>
				</dl>
			</div>
		</div>
	</div>
		...
		<div class="commerce-panel">
			...
			<div class="commerce-panel__content" data-qa-id="commerceBillingAddress">
				<c:if test="<%= commerceOrderDisplayContext.hasViewBillingAddressPermission(permissionChecker, accountEntry) && (billingCommerceAddress != null) %>">
					...
					<c:if test="<%= commerceOrderDisplayContext.isShowCommerceOrderFullAddress() %>">
						<p>
					...
					<c:if test="<%= commerceOrderDisplayContext.isShowCommerceOrderPhoneNumber() %>">
...
<div class="commerce-cta is-visible">
	<portlet:actionURL name="/commerce_open_order/edit_commerce_order" var="editCommerceOrderActionURL">
		<portlet:param name="mvcRenderCommandName" value="/commerce_order/view_commerce_order_details" />
	</portlet:actionURL>
	...
	<c:if test="<%= commerceOrderDisplayContext.isShowProcessQuote() %>">
	...
	<c:if test="<%= commerceOrderDisplayContext.isShowRetryPayment() %>">
		<aui:button cssClass="btn-lg" href="<%= commerceOrderDisplayContext.getRetryPaymentURL() %>" primary="<%= true %>" value="retry-payment" />
	</c:if>

	<liferay-util:dynamic-include key="com.liferay.commerce.order.web#/place_order_detail_cta#" />
</div>
			...
			<div class="commerce-panel__content">
				<dl class="commerce-list">
					...
					<c:if test="<%= (shippingCommerceDiscountValue != null) && (BigDecimal.ZERO.compareTo(shippingCommerceDiscountValue.getDiscountPercentage()) < 0) %>">
						...
						<dd class="text-right"><%= HtmlUtil.escape(commerceOrderDisplayContext.getLocalizedPercentage(shippingDiscountValueWithTaxAmount.getDiscountPercentage(), locale)) %></dd>
					</c:if>
...
<portlet:renderURL var="viewReturnableOrderItemsURL" windowState="<%= LiferayWindowState.POP_UP.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/commerce_order/view_returnable_commerce_order_items" />
...
<liferay-frontend:component
	context='<%=
		HashMapBuilder.<String, Object>put(
			...
			"returnableOrderItemsContextParams", commerceOrderDisplayContext.getReturnableOrderItemsContextParams()
		).put(
