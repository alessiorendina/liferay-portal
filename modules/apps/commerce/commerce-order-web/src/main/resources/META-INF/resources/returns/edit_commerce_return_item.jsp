...
CommerceReturnItem commerceReturnItem = commerceReturnDisplayContext.getCommerceReturnItem();

boolean disabled = ParamUtil.getBoolean(request, "disabled");
%>

<liferay-frontend:side-panel-content
	title='<%= LanguageUtil.format(request, "edit-x", commerceReturnDisplayContext.getCommerceReturnItemId()) %>'
>
	<commerce-ui:panel
		title='<%= LanguageUtil.get(request, "details") %>'
	>
		<aui:form name="commerceReturnItemsFm" onSubmit="event.preventDefault();">
			<aui:input name="commerceReturnItemId" type="hidden" value="<%= commerceReturnDisplayContext.getCommerceReturnItemId() %>" />

			<div class="row">
				<div class="col">
					<aui:input disabled="<%= disabled %>" ignoreRequestValue="<%= true %>" name="quantity" type="text" value="<%= commerceReturnDisplayContext.getFormattedQuantity() %>">
			...
			<div id="<portlet:namespace />commerceReturnItemComments">
				...
				<%
				for (DiscussionComment discussionComment : commerceReturnDisplayContext.getDiscussionComments()) {
				%>
								...
								<div class="row">
									...
									<div class="align-items-center col-auto d-flex">
										<clay:dropdown-actions
											...
											dropdownItems="<%= commerceReturnDisplayContext.getCommerceReturnItemCommentDropdownItemList(discussionComment) %>"
										/>
									</div>
								</div>
							</div>
							...
							<div class="row">
								...
								<div class="col">
									<div class="lfr-discussion-body">
										<aui:input name="className" type="hidden" value="<%= commerceReturnDisplayContext.getCommerceReturnItemClassName() %>" />
...
<liferay-frontend:component
	context='<%=
		HashMapBuilder.<String, Object>put(
			"autocompleteAPIURL", commerceReturnDisplayContext.getListTypeEntriesByExternalReferenceCodeURL()
		).put(
			"autocompleteInitialLabel", commerceReturnDisplayContext.getReturnReasonName()
		).put(
			"autocompleteInitialValue", (commerceReturnItem == null) ? StringPool.BLANK : commerceReturnItem.getReturnReason()
		).put(
			"commerceReturnItemId", commerceReturnDisplayContext.getCommerceReturnItemId()
		).put(
