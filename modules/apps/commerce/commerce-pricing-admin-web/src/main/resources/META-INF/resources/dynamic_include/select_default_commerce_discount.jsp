...
>
	<portlet:actionURL name="/account_admin/edit_account_entry_default_commerce_discount" var="editAccountEntryDefaultCommerceDiscountActionURL" />
	...
	<aui:form action="<%= editAccountEntryDefaultCommerceDiscountActionURL %>" method="post" name="fm">
		...
		<liferay-ui:error exception="<%= DuplicateCommerceChannelAccountEntryRelException.class %>" message="there-is-already-a-discount-defined-for-the-selected-channel" />
		...
		<aui:select label="discount" name="classPK" required="<%= true %>">
			...
			<%
			for (CommerceDiscount commerceDiscount : commerceChannelAccountEntryRelDisplayContext.getCommerceDiscounts()) {
			%>

				<aui:option label="<%= commerceDiscount.getTitle() %>" selected="<%= commerceChannelAccountEntryRelDisplayContext.isEntrySelected(commerceDiscount.getCommerceDiscountId()) %>" value="<%= commerceDiscount.getCommerceDiscountId() %>" />
