...
import com.liferay.commerce.product.web.internal.helper.CPPublisherWebHelper;
import com.liferay.commerce.product.web.internal.util.CPMediaUtil;
...
public class CPPublisherDisplayContext extends BaseCPPublisherDisplayContext {
	...
	public CPDataSourceResult getCPDataSourceResult() throws Exception {
		...
		if (isSelectionStyleDynamic()) {
			cpDataSourceResult = _getDynamicCPDataSourceResult(
				_cpRequestHelper.getRequest(), _searchContainer.getStart(),
	...
	private CPDataSourceResult _getDynamicCPDataSourceResult(
		...
		cpPublisherWebHelper.setOrdering(
			cpQuery, _cpRequestHelper.getPortletPreferences());

		return _cpDefinitionHelper.search(
			_cpRequestHelper.getScopeGroupId(), searchContext, cpQuery, start,
			end);
	}
	...
	private boolean _hasCPContentPortlet() {
		...
		Layout layout = _cpRequestHelper.getLayout();
		...
		for (Portlet portlet : portlets) {
			...
			if (portletId.startsWith(CPPortletKeys.CP_CONTENT)) {
