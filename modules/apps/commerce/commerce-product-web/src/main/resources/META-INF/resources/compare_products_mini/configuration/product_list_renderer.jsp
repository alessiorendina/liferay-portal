...
List<CPContentListRenderer> cpContentListRenderers = cpCompareMiniDisplayContext.getCPContentListRenderers();
%>
...
<aui:fieldset markupView="lexicon">
	<aui:select label="product-list-renderer" name="preferences--cpContentListRendererKey--" onChange='<%= liferayPortletResponse.getNamespace() + "chooseCPContentListRendererKey();" %>'>
		...
		%>
			...
			<aui:option label="<%= HtmlUtil.escape(cpContentListRenderer.getLabel(locale)) %>" selected="<%= key.equals(cpCompareMiniDisplayContext.getCPContentListRendererKey()) %>" value="<%= key %>" />
