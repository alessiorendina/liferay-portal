/**
 * SPDX-FileCopyrightText: (c) 2000 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

package com.liferay.commerce.shipping.engine.fixed.admin.web.internal.constants;

import com.liferay.commerce.constants.CommercePortletKeys;

/**
 * @author Alessio Antonio Rendina
 */
public class CommerceShippingFixedOptionFDSNames {

	public static final String ORDER_TYPES =
		CommercePortletKeys.COMMERCE_SHIPPING_METHOD_ADMIN + "-orderTypes";

	public static final String SHIPPING_FIXED_OPTION_SETTINGS =
		CommercePortletKeys.COMMERCE_SHIPPING_METHOD_ADMIN +
			"-shippingFixedOptionSettings";

	public static final String SHIPPING_FIXED_OPTIONS =
		CommercePortletKeys.COMMERCE_SHIPPING_METHOD_ADMIN +
			"-shippingFixedOptions";

	public static final String TERM_ENTRIES =
		CommercePortletKeys.COMMERCE_SHIPPING_METHOD_ADMIN + "-termEntries";

}