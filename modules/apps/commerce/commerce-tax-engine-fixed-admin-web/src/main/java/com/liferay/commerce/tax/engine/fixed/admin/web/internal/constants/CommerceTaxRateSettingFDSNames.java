/**
 * SPDX-FileCopyrightText: (c) 2000 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

package com.liferay.commerce.tax.engine.fixed.admin.web.internal.constants;

import com.liferay.commerce.constants.CommercePortletKeys;

/**
 * @author Alec Sloan
 */
public class CommerceTaxRateSettingFDSNames {

	public static final String PERCENTAGE_TAX_RATE_SETTING =
		CommercePortletKeys.COMMERCE_TAX_METHOD_ADMIN +
			"-percentageTaxRateSetting";

	public static final String TAX_RATE_SETTING =
		CommercePortletKeys.COMMERCE_TAX_METHOD_ADMIN + "-taxRateSetting";

	public static final String TAX_RATES =
		CommercePortletKeys.COMMERCE_TAX_METHOD_ADMIN + "-taxRates";

}