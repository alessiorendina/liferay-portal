...
public class Query {
	 ...
	 * Invoke this method with the command line:
	 *
	 * curl -H 'Content-Type: text/plain; charset=utf-8' -X 'POST' 'http://localhost:8080/o/graphql' -d $'{"query": "query {orderTypeIdOrderTypeChannels(filter: ___, id: ___, page: ___, pageSize: ___, search: ___, sorts: ___){items {__}, page, pageSize, totalCount}}"}' -u 'test@liferay.com:test'
	 */
	@GraphQLField
			...
			orderTypeChannelResource -> new OrderTypeChannelPage(
				orderTypeChannelResource.getOrderTypeIdOrderTypeChannelsPage(
					...
					_filterBiFunction.apply(
						orderTypeChannelResource, filterString),
					Pagination.of(page, pageSize),
